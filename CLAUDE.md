...
## ⇨ HANDOFF (state as of 2026-06-19 — read this first)
...
- **`docs/heat_TPS/heat_gnn_ieee.tex` / `.pdf`** — the IEEE conference version (current, full 5 pp).
...
**⇨ RESUME HERE:** The conservative **flux-form** surrogate is the gas-case fix and remains the **best aw2 model**. `heat_python/gnn_flux_model.py` (`HeatFluxMPGNN`: antisymmetric face flux + a local source → conservation and a zero baseline BY CONSTRUCTION, killing the quiescent-cell bias; trained via `train_gnn.py --flux`, which zeros y_mean). On a held-out aw2 (gas) forcing, single-step flux gives a **bounded ~120 K mean / ~104 K end** rollout vs the direct model's divergence (25,434 K end), ~240× better; sanity-checked on aw1 (13.7 K). Controlled: same data/hyperparams, only `--flux` differs. Artifacts: `models/heat_flux_aw2.pt` (**BEST**), `heat_direct_aw2.pt` (baseline), `data/aw2_holdout_traj.npz`, `data/rollout_{flux,direct}_aw2.npz`, `figs/flux_aw2_compare.png` + `figs/flux_model_onepager.png` (architecture).

**BPTT finding (2026-06-19): multi-step BPTT OVERFITS the thin aw2 sweep and HURTS held-out.** Added `train_rollout.py --flux` (warm-start from `heat_flux_aw2.pt`). M=2 → 701 K, M=4 → 757 K held-out (both ≫ single-step's ~120 K), even though train/val loss DROPPED — textbook overfitting on only 8 stiff trajectories (aw1's BPTT win used 24). Models `heat_flux_aw2_ms{,2}.pt` kept as evidence (worse). **Single-step + noise is still best for aw2.**

**⇨ NEXT STEP (IN PROGRESS):** generating a **24-forcing** aw2 sweep → `data/aw2_forcing_dataset_n24.npz` (background, ~6 h, started 2026-06-19) to fix the data-quantity bottleneck. When it lands: (1) retrain single-step flux on n24 (`train_gnn.py --data ...n24... --flux --no-pred-gas --noise 0.03 --stayput 1.0`) → eval held-out (`eval_rollout_traj --traj aw2_holdout_traj.npz`); (2) retry BPTT on n24 — more data should let multi-step help instead of overfit. **Adaptive time-stepping is DEFERRED:** same overfit risk on thin data, and `train_adaptive_rollout.py`/`eval_adaptive.py` are single-trajectory/in-sample — a fair held-out adaptive test needs a multi-cadence + multi-forcing dataset and a fine-cadence held-out traj. Revisit after n24. Both papers (markdown whitepaper §3.5/§4.4 + IEEE `heat_gnn_ieee.tex`, full 5 pp) carry the flux result and the BPTT-overfits finding. Repo pushed/synced to origin/main.
