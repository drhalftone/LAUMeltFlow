...
\title{Variable-Length Message-Passing Graph Neural Networks as Surrogates for 1D Finite Element Bead Chain Simulations}
...
Physical simulations of flexible one-dimensional structures such as whips, ropes, cables, and biological filaments are common in physics simulation and animation. The standard formulation models the structure as a chain of mass nodes connected by rod elements, applies local force laws, and integrates forward in time. Even simple cases need small timesteps and many integration steps, so long simulations are expensive even though each step only updates position and velocity by an increment of $\Delta t = 10^{-4}$~s.

Machine learning surrogates for physics simulations offer a way to speed this up. Graph neural networks (GNNs) are a natural fit for mesh-based simulations because the mesh itself is a graph. Nodes are the mesh points, edges are elements, and the local physics at each node depends only on its immediate neighbors. A trained GNN that respects this graph structure can replace the per-timestep numerical computation with a single forward pass.
...
A critical finding during this work was that the training data pipeline matters as much as the model architecture. Early attempts using uniform Cartesian sampling for neighbor positions produced training data dominated by physically unrealistic states, and the resulting models diverged during rollout. Switching to polar coordinate sampling, which constrains neighbor distances to within $\pm 5\%$ of the rest length while varying the angle uniformly, produced training data that reflects the actual distribution of states the model will encounter. With this fix in place, even a simple MLP produced accurate single-step predictions.

The MLP surrogate, however, was trained on a fixed 16-bead chain configuration and would require retraining for any other chain length. This paper describes how we restructured the surrogate to handle any chain length. We restructure the surrogate as a true message-passing GNN with separate node and edge encoders, message functions applied per edge, and ghost masking for chain endpoints. Because all learned functions have shared weights applied per node, the same trained model handles chains of any length. We train on per-bead samples drawn from chains of varying lengths and taper ratios, and we verify that the model generalizes to chain lengths not seen during training.
...
\begin{itemize}
    ...
    \item \textbf{Output head}: a linear layer that projects the final hidden state to the four-dimensional residual state delta.
\end{itemize}

Each bead's forward pass proceeds as follows:
\begin{enumerate}
    \item Normalize input features using stored per-feature mean and standard deviation, computed once from the training set and stored as frozen buffers.
    ...
    \item Apply the output head and denormalize to obtain the residual deltas $[\Delta x, \Delta y, \Delta v_x, \Delta v_y]$.
...
Two architectural choices enable variable-length operation. First, ghost masking ensures that absent neighbors contribute nothing. When bead 0 processes its left edge features (which are zero because no left neighbor exists), the edge encoder and message MLP produce some nonzero output. This output is multiplied by the zero mask, forcing it to zero before aggregation. Second, all learned functions are weight-shared and applied per bead. A 32-bead chain uses the same 80K parameters as an 8-bead chain; only the number of per-bead forward passes changes.

For this paper we use one round of message passing ($K=1$), which matches the one-hop receptive field of the underlying physics. Deeper architectures are possible but would require multi-hop training data or end-to-end rollout training to be effective.

\subsection{Volume-Sampled Per-Bead Training Data}

Rather than generating training data by recording simulation trajectories, we sample single-step physics scenarios uniformly across the reachable state space. Each training sample represents one bead at one instant plus the analytical answer for its state one timestep later. No simulation is ever run during data generation. Each sample is a completely independent random scenario solved by direct evaluation of the physics equations.
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Snapshots from a side-by-side rollout of the reference solver (left, black) and BeadMPGNN (right, red) on a 16-bead chain. Bottom panels show mean position error. The GNN tracks the ground truth through all three phases of motion.}
    \label{fig:snapshots}
\end{figure}
...
Fig.~\ref{fig:eval} shows results for chain lengths $N \in \{8, 10, 12, 16, 20, 24, 32\}$. Short to medium chains ($N \le 16$) track the reference closely with low accumulated error. Longer chains ($N \ge 20$) show more drift, consistent with the longer chain having more compounding per-step errors and more numerous light-mass tip beads. Rod length accuracy (maximum relative error) is in the $10^{-3}$ range across all tested lengths, which is acceptable for the stiff-spring formulation but worse than a full SHAKE projection.
...
Graph neural networks for physical simulation have a growing body of literature. Sanchez-Gonzalez et al.~\cite{sanchez2020learning} demonstrated GNN surrogates for mesh-based particle systems, including fluids and deformable solids. Pfaff et al.~\cite{pfaff2020learning} extended this to dynamic meshes. Battaglia et al.~\cite{battaglia2016interaction} introduced the interaction network formulation that underlies many modern physics GNNs, including the architectural choices we use here.

Hamiltonian and Lagrangian neural networks (Greydanus et al.~\cite{greydanus2019hamiltonian}) take a different approach by learning a scalar energy function and using automatic differentiation to compute forces, which guarantees energy is conserved by design. A parallel research thread in our group is exploring Hamiltonian GNN variants for this same bead chain problem. That work is complementary to the present paper and targets a different failure mode (long-horizon energy drift) with a different architectural mechanism.

Previous work within our research group on the MeltFlow compressible Euler problem demonstrated that simple concatenation works for 1D fixed-topology meshes, informing the architectural comparison in Section V-A.
...
Rollout training and multi-step loss would reduce drift over long rollouts by exposing the model to the effects of its own errors during training. Embedding exact constraint projection into the inference loop, either through a hybrid GNN plus SHAKE step or through a constraint-aware loss term, would improve rod length fidelity. The current one-hop model assumes strict locality; extending to $K > 1$ rounds would require multi-hop training data but could improve accuracy on wave propagation phenomena that span multiple beads per timestep. Finally, integrating the MPGNN surrogate into a larger engineering workflow, for example real-time visualization or optimization loops that require many forward simulations, is the target application that motivates the speed-accuracy tradeoff.
...
The authors thank the University of Kentucky Department of Electrical and Computer Engineering for support.
...
\bibitem{greydanus2019hamiltonian} S. Greydanus, M. Dzamba, and J. Yosinski, ``Hamiltonian neural networks,'' in \emph{Proc. NeurIPS}, 2019.
\end{thebibliography}
