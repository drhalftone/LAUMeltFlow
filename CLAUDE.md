...
**⇨ RESUME HERE:** The conservative **flux-form** surrogate is the gas-case fix, and it WORKS. Built `heat_python/gnn_flux_model.py` (`HeatFluxMPGNN`: antisymmetric face flux + a local source → conservation and a zero baseline BY CONSTRUCTION, which kills the quiescent-cell bias; trained via `train_gnn.py --flux`, which zeros y_mean so a uniform field denormalizes to zero change). On a held-out aw2 (gas) forcing it converts the direct model's divergence (end-of-rollout mean 25,434 K) into a **bounded ~104 K** rollout, ~240× better; sanity-checked on aw1 (13.7 K single-step, far tighter max than the direct model). Controlled comparison: same data/hyperparams, only `--flux` differs. Artifacts: `models/heat_flux_aw2.pt` (best), `models/heat_direct_aw2.pt` (baseline), `data/aw2_holdout_traj.npz`, `data/rollout_{flux,direct}_aw2.npz`, `figs/flux_aw2_compare.png`. **NEXT CONCRETE STEP: multi-step BPTT on the flux model** — add a `--flux` path to `train_rollout.py` (build `HeatFluxMPGNN` + zero the y_mean stat used in `mesh_step`), warm-start from `heat_flux_aw2.pt`, to turn bounded→accurate; then add time-step awareness + an adaptive schedule for the surface transient. Flux result is **single-step only** so far. Both papers carry it: markdown whitepaper §3.5/§4.4 and the IEEE `docs/heat_TPS/heat_gnn_ieee.tex` (now a full 5-page draft, all sections written, bib filled except the `own_symposium` placeholder). Repo pushed/synced to origin/main.
