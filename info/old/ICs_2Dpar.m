...
P_a = 1.01325e1;                        % -> Ambient pressure [Pa]
U = zeros(n_prp,npt(1),npt(2));         % Allocate 'U'
parfor i = 1:npt(1)                     % Assign ambient properties
    U_slc = zeros(n_prp,1,npt(2));
    for j = 1:npt(2)
        U_slc(:,1,j) = [rho_a,u_a,v_a,P_a];   
    end
    U(:,i,:) = U_slc;
end
...
if (opt == 1) 
    ...
    P_c = 1.01325e1;                    % -> Region pressure [Pa]
