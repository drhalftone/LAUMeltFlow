...
[U,phi] = riemann_exact(gam,x_0,x,t,U_L,U_R);
% phi(1:length(x)) = 1;
...
%===== Pre-Processing =====%
    ...
    a4 = sqrt(gamL*p4/rho4);
    a1 = sqrt(gamR*p1/rho1);
    ...
    while(abs(err) >= abs(tol))
        it = it + 1;
        rPrv = r;
        r = (1-damping)*r + damping*p4/p1*(1 - ((gamL-1)*a1/a4*(r-1))/sqrt(2*gamR*(2*gamR+(gamR+1)*(r-1))))^(2*gamL/(gamL-1));
        err = r - rPrv;
    end
    p2op1 = r;
    ...
    us = a1*sqrt((gamR+1)/(2*gamR)*(r-1)+1)
    p2 = p2op1*p1;
    u2 = a1/gamR*(r-1)*sqrt( (2*gamR/(gamR+1))/(r+(gamR-1)/(gamR+1)) )
